    ...
    while (!exitRequested) {
      ...
      if (choice === "1" || choice === "Process Input") {
        try {
          ...
          // Check if user only has the default template without adding actual content
          const isOnlyTemplate =
            content.includes("These are the Test Cases") &&
            content.includes("Make sure above test cases are passed") &&
            !content.match(/```[\s\S]+```/) && // No code blocks
            content
              .split("\n")
              .filter(
                (line) =>
          ...
          if (isOnlyTemplate) {
            renderMenu(
              `${this.ui.colors.yellow}⚠️ Input file appears to only contain the template.\n${this.ui.colors.blue}📝 Please add your test cases and code to input.txt${this.ui.colors.reset}`
            );
            await waitForKeyPress();
            isProcessing = false;
            continue;
