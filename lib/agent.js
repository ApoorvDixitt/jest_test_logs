...
class BrahmaAgent {
  ...
  async run() {
    ...
    try {
      ...
      if (testCases.length === 0) {
        const errorMsg = "No test cases found. Agent requires test cases to function.";
      ...
      if (codeTemplates.length > 0) {
        ...
        this.ui.showWarning("   No source templates found (API will generate from scratch)");
      }
  ...
  async validatePaths() {
    ...
    if (!this.sourcePathExists) {
      ...
      this.debug("DBG006", "  ⚠️ Source path not found (API will generate code)");
      this.ui.showWarning("   ⚠️ Source path not found - API will generate code from scratch");
    }
