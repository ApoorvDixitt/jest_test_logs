...
const BrahmaAgent = require("./agent");
  ...
  async startFileModeInteractive() {
    ...
    const options = [
      ...
      },
      {
        key: "2",
        label: "Agent Mode",
        description: "Autonomous agent - scan codebase & generate solutions",
      },
      { key: "3", label: "Exit Brahma", description: "Close the application" },
    ];
    ...
    while (!exitRequested) {
      ...
      } else if (choice === "2" || choice === "Agent Mode") {
        // Agent Mode
        try {
          if (isProcessing) {
            renderMenu(
          ...
          // Clear screen for agent mode
          process.stdout.write("\x1b[2J\x1b[3J\x1b[H");

          console.log(
            `${this.ui.colors.bright}${this.ui.colors.magenta}🤖 BRAHMA AGENT MODE${this.ui.colors.reset}`
          );
          console.log(
            `${this.ui.colors.dim}Autonomous codebase analysis & solution generation${this.ui.colors.reset}`
          );
          console.log("");

          // Create and run the agent
          const agent = new BrahmaAgent(this.ui, this.fetch);
          const result = await agent.run();

          if (result.success) {
            requestCount++;
            renderMenu(
              `${this.ui.colors.green}✅ Agent mode completed successfully!\n📝 Output written to output.txt${this.ui.colors.reset}`
            );
          } else {
            renderMenu(
              `${this.ui.colors.red}❌ Agent mode failed: ${
                result.error || result.message
              }${this.ui.colors.reset}`
            );
          }
          ...
          let errorMsg = `${this.ui.colors.red}❌ Agent mode failed: ${error.message}${this.ui.colors.reset}`;
          errorMsg += `\n${this.ui.colors.yellow}💡 Check the output.txt for more details.${this.ui.colors.reset}`;
